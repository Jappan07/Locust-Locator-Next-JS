...
const fetchData = async () => {
  let data = []
  await axios.get("https://services5.arcgis.com/sjP4Ugu5s0dZWLjd/arcgis/rest/services/Swarms_Public/FeatureServer/0/query?where=%20(LOCPRESENT%20%3D%201%20OR%20LOCPRESENT%20%3D%202)%20&outFields=STARTDATE,TmSTARTDAT,FINISHDATE,EXACTDATE,LOCPRESENT&outSR=4326&f=json")
    .then(response => {
      response.data.features.map(pos => {
        console.log(pos)
        data.push({
          longitude: isNaN(pos.geometry.x) ? null : pos.geometry.x,
          latitude: isNaN(pos.geometry.y) ? null : pos.geometry.y
        })
      })
    })
  return data
...
const Tracking = () => {
  ...
  const init = (locationData) => {
    ...
    var east = 89.0;
    var north = 35.0;
    Cesium.Ion.defaultAccessToken = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJqdGkiOiJiNTQ0MzhmMy0xNmNmLTRiMjQtODUzNC1jZmZiNDUzNGUwYTciLCJpZCI6NjcxMTAsImlhdCI6MTYzMTU0NDExNn0.TZvYpD3fdpWSWmY3iMPNtxggZVqgnZL2Pdc5yhcDdBY"
  ...
  const onFormSubmitHandler = (event) => {
    ...
    if (lat < -90 || long < -180 || lat > 90 || long > 180) {
      ...
      let newData = (Math.random() * 100).toFixed(2)
      setPredictedData(newData / 100)
      setDisplayMessage("Probability = " + newData + "%")
  ...
  return (
    <>
      <div id="cesium">
        ...
        <div id="cesium-viewer-toolbar"><button className={"cesium-button"}>Locust Movement</button></div>
